...
function getToolLabel(toolInvocation: { toolName: string; args?: Record<string, unknown> }): string {
  ...
  if (toolName === "str_replace_editor" && fileName) {
    switch (command) {
      ...
      case "str_replace": return `Edited ${fileName}`;
      case "insert": return `Edited ${fileName}`;
      case "view": return `Viewed ${fileName}`;
    }
  }
  ...
  if (toolName === "file_manager") {
    ...
    switch (command) {
      case "rename": {
        ...
        return newName ? `Renamed to ${newName}` : "Renamed file";
      }
      case "delete": return fileName ? `Deleted ${fileName}` : "Deleted file";
    }
  }
